...
const DiskMemory: React.FC<DiskProps> = ({
  ...
  const isResetting = useRef(false); // Referência para controlar o reset

  useEffect(() => {
    if (reset && !isResetting.current) {
      // Verifica se é um novo reset e executa apenas uma vez
      ...
      // Define um intervalo de 1 segundo para desabilitar o estado de reset
      setTimeout(() => {
